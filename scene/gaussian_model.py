...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, gaussian_dim : int = 3, time_duration: list = [-0.5, 0.5], rot_4d: bool = False, force_sh_3d: bool = False, sh_degree_t : int = 0, dist_thres : int = 10):
        ...
        self.denom = torch.empty(0)                 # 密度控制中，梯度的分母
        self.optimizer = None
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # opacities = inverse_sigmoid(0.85 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda"))
    ...
    def training_setup(self, training_args):
        ...
        if self.gaussian_dim == 4: # TODO: tune time_lr_scale
            if training_args.position_t_lr_init < 0:            # 设置时间维度上的学习率 
    ...
    def reset_opacity(self):                                                                                        # 重置透明度大于0.01的点
    ...
    def prune_points(self, mask):
        valid_points_mask = ~mask       # ~ 运算符是 Python 中的按位取反运算符。当应用于布尔掩码时，它会翻转值，即将 True 转换为 False
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, grads_t, grad_t_threshold, N=2):
        ...
        # Extract points that satisfy the gradient condition （梯度值大的且缩放因子较大的gaussian要分裂）
        ...
        # selected_pts_mask2 = torch.logical_and(dist > (self.dist_thres * scene_extent),
        ...
        new_scaling = self.scaling_inverse_activation(self.get_scaling[selected_pts_mask].repeat(N,1) / (0.8*N))        # split的精髓
        ...
        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=bool)))  # #创建一个修剪掩码，标记在分裂前应分裂的原始点的位置
    ...
    def guided_densification(self, selected_pixel, viewpoint_cam, render_depth, gt_depth, gt_image):
        
        def pix2ndc(v, S):
            return (v * 2.0 + 1.0) / S - 1.0

        # depth inpainting using MDE depth as GT
        num_intervals = 500
        gt_depth_interval = [0,]
        gt_interval_indices = []
        ratio = []
        ...
        for i in range(num_intervals):
            gt_depth_interval.append(torch.quantile(gt_depth, (i + 1) / num_intervals))
            gt_interval_indices.append(torch.where((gt_depth_interval[i + 1] >= gt_depth) & (gt_depth > gt_depth_interval[i])))

            gt_depth_map_interval_value = gt_depth[gt_interval_indices[i]]
            render_depth_map_interval_value = render_depth[gt_interval_indices[i]]

            render_depth_nonzero_indices = torch.nonzero(render_depth_map_interval_value)
            ratio.append(torch.mean(render_depth_map_interval_value[render_depth_nonzero_indices] / gt_depth_map_interval_value[render_depth_nonzero_indices]))

        # fused_point_cloud
        gt_depths = gt_depth[selected_pixel[:,0], selected_pixel[:,1]]
        depths = torch.zeros_like(gt_depths)
        
        for i in range(len(gt_depths)):
            for j in range(num_intervals):

                if gt_depth_interval[j]<= gt_depths[i] <= gt_depth_interval[j + 1]:
                    depths[i] = gt_depths[i] * ratio[j]
                else:
                    continue
        ...
        u = selected_pixel[:,0] # hight y
        v = selected_pixel[:,1] # weidth  x 
        ...
        targetPz = (depths + depths/100*(randomdepth))
        ...
        xyz = worldpoint[:, :3] 
        fused_point_cloud = xyz.float().cuda()

        # fused_color
        rgbs = gt_image[:, selected_pixel[:,0], selected_pixel[:,1]].permute(1,0)
        fused_color = RGB2SH(rgbs.float().cuda())
        ...
        print("Number of points under guided densification: ", fused_point_cloud.shape[0])

        # dist2 = torch.clamp_min(distCUDA2(xyz.float().cuda()), 0.0000001)
        # scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        scales = torch.full((xyz.shape[0], 3), 0.001).cuda()
        rots = torch.zeros((fused_point_cloud.shape[0], 4), device="cuda")
        rots[:, 0] = 1
        if self.gaussian_dim == 4:
            dist_t = torch.zeros_like(fused_times, device="cuda") + (self.time_duration[1] - self.time_duration[0]) / 5
        ...
        if self.gaussian_dim == 4:
            new_t = nn.Parameter(fused_times.requires_grad_(True))
            new_scaling_t = nn.Parameter(scales_t.requires_grad_(True))
            if self.rot_4d:
                new_rotation_r = nn.Parameter(rots_r.requires_grad_(True))
