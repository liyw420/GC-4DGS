...
def normalize_rgb(input, mean=None, std=None):
    input_mean = torch.mean(input, dim=2, keepdim=True) if mean is None else mean
    input_std = torch.std(input, dim=2, keepdim=True) if std is None else std
    return (input - input_mean) / (input_std + 1e-2*torch.std(input.reshape(-1)))
...
def patchify_rgb(input, patch_size):
